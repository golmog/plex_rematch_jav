...
-   **기능 개선/추가**: 로깅 관련 설정 추가


**2025-12-20**

-   **기능 개선**: 인터랙션 모드에서 기본값 지정, Uncensored 용 로직 개선 등


**2025-12-01**

-   **기능 개선**: `--search` 옵션에서 `%`, `_` 등의 SQL 와일드카드 사용 가능: 사용자가 와일드카드를 직접 입력했다면 그대로 사용, 아니면 양쪽에 % 추가


**2025-11-21**

-   **신규 기능**: --clean-db 모드 추가: Plex 라이브러리에 더 이상 존재하지 않는 아이템의 기록을 스크립트 DB에서 정리하고 최적화(VACUUM)합니다.
-   **신규 기능**: --analyze-missing 모드 추가: 미디어 분석이 완료되지 않은 항목을 찾아 분석 명령을 내립니다.


**2025-11-08**

-   **신규 기능**: Uncensored 모드 추가. `--uncensored`(`--uncen`) 옵션: Uncensored 전용 섹션 처리시 지정


**2025-09-26**

-   **기능 개선**: `--scan-full` 모드 스캔시 세션 저장 기능 추가: 중단된 스캔을 이어서 하거나 새로 시작 가능
